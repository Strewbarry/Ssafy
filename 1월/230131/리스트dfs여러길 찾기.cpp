...
vector<int> v[8];  //v[from] : from에서 갈 수 있는 to 들을 저장할 vector 생성
int used[8]; // used, visited  >> 해당 번호를 들린적이 있는가?
...
void dfs(int now) {
	//2. 기저조건 확인 (option)
	if (now == 7) { // now == index : index 지점까지만 탐사하고 싶다.
	...
	for (int i = 0; i < v[now].size(); i++)
	{
		...
		//1. 재귀 시작
		dfs(to);
		used[to] = 0; // uesd 복구 : 현재경로와 완벽하게 일치(다양한 경로를 찾는다)
					  //	 복구X : 모든 점을 한번씩만 들려서 길을 찾는다 
